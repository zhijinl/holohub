...
def convert_to_dcm(
    ...
    meta_ds.MediaStorageSOPClassUID = pydicom.uid.CTImageStorage
    ...
    for frame_id in range(inp.shape[0]):
        ...
        if local_save:
            ...
            save_path = os.path.join(local_save_dir, f"slice-{frame_id:03}.dcm")
            print(f"saveing DICOM data: {save_path}")
            slice_ds.save_as(save_path, write_like_original=False)
...
class InputOp(Operator):
    ...
    def __scale_sinogram(
        ...
        if any(
            ...
            return skimage.transform.resize(
                sino,
                output_shape=(
                    sinogram_size_x, sinogram_size_y, sinogram_size_z
                )
            )
        else:
            return sino
    ...
    def compute(self, op_input, op_output, context):
        ...
        op_output.emit({"sino": sino_emit, "angles": angles_emit}, "out")
...
class FDKReconOp(Operator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fov_size_lu = [fov_x_lu, fov_y_lu, fov_z_lu]
        self.fov_size_voxels = [fov_x_voxels, fov_y_voxels, fov_z_voxels]

        self.detector_size_pixels = [detector_x_pixels, detector_y_pixels]
        self.detector_size_lu = [detector_x_lu, detector_y_lu]

        self.sod = src_orig_dist
        self.sdd = src_det_dist

        self.final_recon = np.zeros(self.fov_size_voxels, dtype=np.float32)
    ...
    def create_projector(self, angles):
        ConstantImageSourceType = rtk.ConstantImageSource[
            rtk.CudaImage[itk.F, 3]]

        # Defines the RTK geometry object
        geometry = rtk.ThreeDCircularProjectionGeometry.New()
        sid = self.sod  # source to isocenter distance
        sdd = self.sdd  # source to detector distance
        for angle in angles:
            geometry.AddProjection(sid, sdd, angle)

        # Projector generator
        projector = ConstantImageSourceTypeself..New( = .New()
        origin = [
            -self.detector_size_lu[0]/2.0,
            -self.detector_size_lu[1]/2.0,
            -(self.sdd - self.sod)
        ]
        sizeOutput = [
        ...
        spacing = [
            ...
            1.,  # This field does not matter
        ]
        projector.SetOrigin(origin)
        projector.SetSpacing(spacing)
        projector.SetSize(sizeOutput)
        projector.SetConstant(0.)

        # Volume
        volume = ConstantImageSourceType.New()
        origin = [
            -self.fov_size_lu[0]/2.0,
            -self.fov_size_lu[1]/2.0,
            -self.fov_size_lu[2]/2.0,
        ]
        sizeOutput = [
            self.fov_size_voxels[0],
            self.fov_size_voxels[1],
            self.fov_size_voxels[2],
        ]
        spacing = [
        ...
        volume.SetSize(sizeOutput)
        volume.SetConstant(0.)

        return projector, volume, geometry

    def compute(self, op_input, op_output, context):
        in_message = op_input.receive("in")

        # Get sinogram in numpy format
        sino = cp.asarray(in_message["sino"]).get()
        sino_itk = itk.image_from_array(sino)

        # Get angles in numpy format
        angles = cp.asarray(in_message["angles"]).get()

        # Create projector and volume buffers
        projector, volume, geometry = self.create_projector(angles)

        # Copy sinogram data to GPU
        projector.Update()
        sino_itk_cuda = rtk.CudaImage[itk.F, 3].New()
        sino_itk_cuda.SetPixelContainer(sino_itk.GetPixelContainer())
        sino_itk_cuda.CopyInformation(projector.GetOutput())
        sino_itk_cuda.SetBufferedRegion(sino_itk.GetBufferedRegion())
        sino_itk_cuda.SetRequestedRegion(sino_itk.GetRequestedRegion())

        # Start FDK recon
        FDKGPUType = rtk.CudaFDKConeBeamReconstructionFilter
        feldkamp = FDKGPUType.New()
        feldkamp.SetInput(0, volume.GetOutput())
        feldkamp.SetInput(1, sino_itk_cuda)
        feldkamp.SetGeometry(geometry)

        feldkamp.Update()
        recon_itk_cuda = feldkamp.GetOutput()

        # Convert recon to numpy array
        recon_itk = rtk.Image[itk.F, 3].New()
        recon_itk.SetPixelContainer(recon_itk_cuda.GetPixelContainer())
        recon_itk.CopyInformation(recon_itk_cuda)
        recon_itk.SetBufferedRegion(recon_itk_cuda.GetBufferedRegion())
        recon_itk.SetRequestedRegion(recon_itk_cuda.GetRequestedRegion())
        recon_arr = itk.array_from_image(recon_itk)

        self.final_recon += recon_arr
        ...
        final_recon = cp.asarray(self.final_recon).transpose(1, 0, 2).copy()
        recon_complete = cp.asarray([int(self.counter == self.num_angles)])

        op_output.emit({"vis": vis}, "out_vis")
        op_output.emit(
            {
                "recon": final_recon,
...
class VolumeDenoisingOp(Operator):
    ...
    Implemented using polygraphy APIs for TRT inference.
    """
    ...
    def compute(self, op_input, op_output, context):
        ...
        if cp.asarray(recon_complete)[0]:
            ...
            with self.runner as runner:
                ...
                output = runner.infer(
                    feed_dict={self.model_input_name: input_data}
                )
...
class DICOMSenderOp(Operator):
    ...
    def setup(self, spec: OperatorSpec):
        ...
        # Initialise Application Entity and request CT storage
        self.ae = AE()
        ...
        if assoc.is_established:
            ...
            for slice_id, slice_ds in enumerate(dcm_slices):
                ...
                if status:
                    print(
                        "\r",
                        "slice {0:03} sent, status: 0x{1:04x}".format(
                            slice_id + 1,
                            status.Status
                        ),
                        end="",
...
class FDKReconApp(Application):
    def compose(self):
        ...
        pixel_spacing_x = (
            float(self.kwargs("geometry")["fov_x_lu"]) /
            self.kwargs("geometry")["fov_x_voxels"]
        )
        pixel_spacing_y = (
            float(self.kwargs("geometry")["fov_y_lu"]) /
            self.kwargs("geometry")["fov_y_voxels"]
        )
        slice_thickness = (
            float(self.kwargs("geometry")["fov_z_lu"]) /
            self.kwargs("geometry")["fov_z_voxels"]
        )

        # Send data to DICOM server when recon is complete
        dcm_sender_op_volume = DICOMSenderOp(
            self,
            ip=self.kwargs("dicom")["ip"],
            port=self.kwargs("dicom")["port"],
            pixel_spacing_x=pixel_spacing_x,
            pixel_spacing_y=pixel_spacing_y,
            slice_thickness=slice_thickness,
            local_save=self.kwargs("dicom")["local_save"],
            local_save_dir=self.kwargs("dicom")["local_save_dir"],
        )

        # Send data to DICOM server when volume denoising is complete
        ...
        self.add_flow(
            volume_denoising_op, dcm_sender_op_volume_denoised,
            {("out_volume_denoised", "in")}
        )
